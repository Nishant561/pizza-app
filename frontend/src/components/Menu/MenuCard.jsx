...
function MenuCard({item}) {
  return (
    <>
                      <div className='w-[265px] sh h-[450px] rounded-md p-3  relative'>
                      <div className="image-container p-3 w-full overflow-hidden flex justify-center h-1/2 ">
                              <img className='w-[65%] object-center full object-contain' src={item.image} />
                      </div>
                      <div className="pizza-description gap-4 flex flex-col items-center  p-3">
                                <h1 className='capitalize font-medium  text-2xl'>{item.name}</h1>
                                <p className='text-xl text-zinc-600'>{item.size}</p>
                                <div className="price-container absolute bottom-1 p-3 flex w-full items-center justify-between text-[#FE5F1E]">
                                  <p className='text-gold-700 text-xl font-bold'>${item.price}</p>
